...
class WeixinCtrl @Inject()(weixinService: WeixinService) extends Controller with BaseController {
  def get(signature: String, echostr: String, timestamp: String, nonce: String) = Action { request =>
    ...
    if (weixinService.validateSign(timestamp, nonce) == signature) {
      Ok(echostr)
    } else {
      BadRequest
    }
  ...
  def post = Action(parse.tolerantText) { request =>
    ...
    val body =
      if (encryptType.contains("aes")) {
        weixinService.descryptMsg(msgSignature.get, timestamp.get, nonce.get, request.body)
      } else {
        request.body
      }

    val node = scala.xml.XML.loadString(body)
    val resp = OrdinaryMessage.msgType(node) match {
  ...
  private def getContent(node: Elem, content: String = ""): String = {
    val msg = OrdinaryMessage(node)
